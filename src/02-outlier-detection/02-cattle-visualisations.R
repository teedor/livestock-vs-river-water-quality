# The script's primary intent is to visually analyze and present the distribution of cattle counts across different wards, 
...
# dispersion, and potential outliers within each ward's cattle count data. 
...
# Specify the path to the input CSV file which contains the cattle data along with an outlier flag for each record.
input_file_path <- "../../working-csv-data/outliers/01-cattle-outliers.csv"
output_file_path <- "../../visualisations/outliers/ward_cattle_boxplots.png"
...
# The aes function defines the aesthetic mappings, using 'ward' as the x-axis and 'Cattle' counts as the y-axis.
...
ggplot(data_for_boxplot, aes(x = ward, y = Cattle, fill = ward)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Boxplot of Cattle Counts by Ward",
       x = "Ward",
       y = "Cattle Count") +
  scale_fill_discrete(name = "Ward")

# Save the generated plot to a file, specifying its dimensions.
# This function creates a PNG image file named 'ward_cattle_boxplots.png',
