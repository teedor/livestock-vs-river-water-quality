# The script's primary intent is to visually analyze and present the distribution of sheep counts across different wards, 
...
# dispersion, and potential outliers within each ward's sheep count data. 
...
# Specify the path to the input CSV file which contains the sheep data along with an outlier flag for each record.
input_file_path <- "../../working-csv-data/outliers/01-sheep-outliers.csv"
output_file_path <- "../../visualisations/outliers/ward_sheep_boxplots.png"
...
# The aes function defines the aesthetic mappings, using 'ward' as the x-axis and 'Sheep' counts as the y-axis.
...
ggplot(data_for_boxplot, aes(x = ward, y = Sheep, fill = ward)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Boxplot of Sheep Counts by Ward",
       x = "Ward",
       y = "Sheep Count") +
  scale_fill_discrete(name = "Ward")

# Save the generated plot to a file, specifying its dimensions.
# This function creates a PNG image file named 'ward_sheep_boxplots.png',
