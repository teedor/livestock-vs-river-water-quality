# The script's primary intent is to visually analyze and present the distribution of pigs counts across different wards, 
...
# dispersion, and potential outliers within each ward's pigs count data. 
...
# Specify the path to the input CSV file which contains the pigs data along with an outlier flag for each record.
input_file_path <- "../../working-csv-data/outliers/01-pigs-outliers.csv"
output_file_path <- "../../visualisations/outliers/ward_pigs_boxplots.png"
...
# The aes function defines the aesthetic mappings, using 'ward' as the x-axis and 'Pigs' counts as the y-axis.
...
ggplot(data_for_boxplot, aes(x = ward, y = Pigs, fill = ward)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Boxplot of Pigs Counts by Ward",
       x = "Ward",
       y = "Pigs Count") +
  scale_fill_discrete(name = "Ward")

# Save the generated plot to a file, specifying its dimensions.
# This function creates a PNG image file named 'ward_pigs_boxplots.png',
