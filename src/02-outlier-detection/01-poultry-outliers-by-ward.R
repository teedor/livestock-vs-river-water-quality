# The script's primary goal is to identify outliers in poultry counts across different wards, where outliers are defined as counts more than 
...
# This methodical approach ensures a focused analysis on significant deviations in poultry counts,
...
output_file_path <- "../../working-csv-data/outliers/01-poultry-outliers.csv"
...
# an outlier if it is more than 8 times the ward's median poultry count.
outlierThreshold <- 10

significantCountThreshold <- 100000
...
# Filter the data to exclude rows where the poultry count is less than 5,
# as these are not considered relevant for outlier analysis.
filtered_data <- livestock_data %>% filter(Poultry >= 5)

# Calculate the median poultry count for each ward. This provides a baseline
# for identifying what is considered typical within each ward.
median_counts <- filtered_data %>%
  group_by(ward) %>%
  summarise(MedianPoultry = median(Poultry))
...
# the poultry count is both more than 8 times the ward's median and exceeds the significant threshold count.
...
joined_data <- joined_data %>%
  mutate(OutlierFlag = ifelse(Poultry > outlierThreshold * MedianPoultry & Poultry > significantCountThreshold, TRUE, FALSE))

# Prepare the final dataset for export by selecting only the relevant columns:
# ward, Year, Poultry, and OutlierFlag.
output_data <- joined_data %>%
  select(ward, Year, Poultry, OutlierFlag)
