# The script's primary goal is to identify outliers in pigs counts across different wards, where outliers are defined as counts more than 
...
# This methodical approach ensures a focused analysis on significant deviations in pigs counts,
...
output_file_path <- "../../working-csv-data/outliers/01-pigs-outliers.csv"
...
# an outlier if it is more than 8 times the ward's median pigs count.
outlierThreshold <- 8

significantCountThreshold <- 20000
...
# Filter the data to exclude rows where the pigs count is less than 5,
# as these are not considered relevant for outlier analysis.
filtered_data <- livestock_data %>% filter(Pigs >= 5)

# Calculate the median pigs count for each ward. This provides a baseline
# for identifying what is considered typical within each ward.
median_counts <- filtered_data %>%
  group_by(ward) %>%
  summarise(MedianPigs = median(Pigs))
...
# the pigs count is both more than 8 times the ward's median and exceeds the significant threshold count.
...
joined_data <- joined_data %>%
  mutate(OutlierFlag = ifelse(Pigs > outlierThreshold * MedianPigs & Pigs > significantCountThreshold, TRUE, FALSE))

# Prepare the final dataset for export by selecting only the relevant columns:
# ward, Year, Pigs, and OutlierFlag.
output_data <- joined_data %>%
  select(ward, Year, Pigs, OutlierFlag)
